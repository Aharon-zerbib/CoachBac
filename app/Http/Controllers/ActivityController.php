...
class ActivityController extends Controller
{
    ...
    public function weeklyStats(Request $request, GoogleFitService $googleFitService)
    {
        $user = $request->user();
        $cacheKey = "weekly_stats_v2_{$user->id}";

        $stats = \Illuminate\Support\Facades\Cache::remember($cacheKey, 60, function () use ($user, $googleFitService) {
            return $googleFitService->getWeeklySteps($user);
        });
    ...
    public function stats(Request $request, GoogleFitService $googleFitService)
    {
        ...
        $cacheKey = "stats_v2_{$user->id}_{$date}";

        $result = \Illuminate\Support\Facades\Cache::remember($cacheKey, 60, function () use ($user, $googleFitService, $date) {
            $googleData = null;
